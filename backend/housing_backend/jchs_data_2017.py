...
from housing_backend.models import JCHSData
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': key, 'datapoint': 'United States', 'value': val, 'valuetype': value_type }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': ser_ix[1] + ' Households', 'datapoint': key, 'value': val, 'valuetype': 'count' }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': ser_ix[0], 'datapoint': key, 'value': val, 'valuetype': 'count' }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': key + ' Households by Nativity', 'datapoint': ser_ix, 'value': val, 'valuetype': 'count' }
                yield body
        ...
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                if pd.isnull(val):
                    continue
                time = datetime(ix, 1, 1, tzinfo=pytz.utc)
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': 'Average Real Household Incomes by Income Quintile', 'datapoint': ser_ix, 'value': val, 'valuetype': '2015 Dollars' }
                yield body
        ...
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                if ser_ix[0] == 'All Households':
                    dp = ser_ix[0]
                time = datetime(ix, 1, 1, tzinfo=pytz.utc)
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': 'Homeownership Rates by Age, Race/Ethnicity, and Region', 'datapoint': dp, 'value': val, 'valuetype': 'Percent' }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': ser_ix[1] + ' ' + key, 'datapoint': dp, 'value': val, 'valuetype': 'count' }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                time = datetime(2015, 1, 1, tzinfo=pytz.utc)
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': key, 'datapoint': dp, 'value': val, 'valuetype': '2015 dollars' }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': key, 'datapoint': dp, 'value': val, 'valuetype': '2016 dollars' }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                dp = ix.strip()
                time = datetime(2015, 1, 1, tzinfo=pytz.utc)
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': key, 'datapoint': dp, 'value': val, 'valuetype': value_type }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                dp = ix.strip()
                time = datetime(ser_ix, 1, 1, tzinfo=pytz.utc)
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': key, 'datapoint': dp, 'value': val, 'valuetype': 'ratio' }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': 'Continuum of Care Homelessness', 'datapoint': dp, 'value': val, 'valuetype': 'count' }
                yield body
    ...
    def generate_json(self):
        for ix, row in self.df.iterrows():
            ...
            for ser_ix, val in row.iteritems():
                ...
                body = { 'time': time.astimezone(pacific), 'source': self.source, 'datatype': data_type, 'datapoint': dp, 'value': val, 'valuetype': value_type }
                yield body
